...
class ResetPasswordController extends AbstractController
{
     ...
     */
    #[Route('/verification_email', name: 'app_verification_email')]
    public function checkEmail(): Response
    {
        // Génère un faux jeton si l'utilisateur n'existe pas ou si quelqu'un a accédé directement à cette page
        // Pas de vérification de si un utilisateur a été trouvé avec l'email ou non pas
    ...
    public function reset(Request $request, UserPasswordHasherInterface $passwordHasher, TranslatorInterface $translator, ?string $token = null): Response
    {
        if ($token) {
            ...
            // afin d'éviter que l'URL ne soit chargée dans un navigateur et que le jeton ne soit transmis à un JavaScript tiers
        ...
        // Le jeton est valide ; l'utilisateur peut modifier son mot de passe
        $form = $this->createForm(ChangePasswordFormType::class);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            // Un jeton de réinitialisation de mot de passe n'est utilisé qu'une seule fois
            ...
            // La session est supprimée après la modification du mot de passe
    ...
    private function processSendingPasswordResetEmail(string $emailFormData, MailerInterface $mailer, TranslatorInterface $translator): RedirectResponse
    {
        ...
        $email = (new TemplatedEmail())
            ->from(new Address('mailer@livrosphere.com', 'Livrosphere'))
